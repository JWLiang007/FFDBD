...
class prepro_cls_DatasetBD(torch.utils.data.dataset.Dataset):
    def __init__(
            ...
            poison_idx:
        Sequence,  # one-hot to determine which image may take bd_transform
            add_details_in_preprocess: Optional[bool] = True,
            clean_image_pre_transform: Optional[Callable] = None,  #np.array,
            bd_image_pre_transform: Optional[Callable] = None,
            bd_label_pre_transform: Optional[Callable] = None,
            end_pre_process:
        Optional[
            Callable] = None,  # lambda img_list : [Image.fromarray(img.astype(np.uint8)) for img in img_list],
            ori_image_transform_in_loading: Optional[Callable] = None,
            ori_label_transform_in_loading: Optional[Callable] = None,
            load_img=True):
        logging.info(
            'dataset must have NO transform in BOTH image and label !')
        ...
        assert len(poison_idx) == full_dataset_without_transform.get_vlen()
        self.prepro_backdoor()
        
        self.valid_idx = None
        
    def set_valid_idx(self,valid_idx):
    ...
    def prepro_backdoor(self):
        ...
        if self.bd_image_pre_transform is not None:
            self.dataset.bd_image_pre_transform = self.bd_image_pre_transform
        for original_idx in tqdm(range(self.dataset.get_vlen()),
                                 desc=f'pre-process bd dataset'):
            # for original_idx, content in enumerate(tqdm(self.dataset, desc=f'pre-process bd dataset')):
            if self.load_img:
                img, label = self.dataset.get_item(original_idx)
            else:
                img = None
                label = self.dataset.get_label(original_idx)

            if self.clean_image_pre_transform is not None and self.poison_idx[
                    original_idx] == 0:

                img = self.clean_image_pre_transform(img)

            if self.bd_image_pre_transform is not None and self.poison_idx[
                    original_idx] == 1 and self.load_img:

                img = self.bd_image_pre_transform(img, label, original_idx)

            original_label = deepcopy(label)

            if self.bd_label_pre_transform is not None and self.poison_idx[
                    original_idx] == 1:
        ...
        self.dataset.update_vlabel(self.targets,self.poison_indicator)

    def __getitem__(self, item):

        if self.valid_idx is not None:
            item = self.valid_idx[item]
        bd_item = self.dataset.get_item(item)
        ...
        # if self.add_details_in_preprocess:
        #     return img, label, self.original_index[
        #         item], self.poison_indicator[item], self.original_targets[item]
        # else:
        return bd_item
    ...
    def subset(
        ...
        if inplace:
            self.data = [self.data[ii] for ii in chosen_index_list
                         ]  #self.data[chosen_index_array]
            self.targets = self.targets[chosen_index_list]
            if self.add_details_in_preprocess:
                self.original_index = self.original_index[chosen_index_list]
                self.poison_indicator = self.poison_indicator[
                    chosen_index_list]
                self.original_targets = self.original_targets[
                    chosen_index_list]
            if not memorize_original:
                self.dataset, self.poison_idx = None, None
            self.dataset.filter_bd_imgs(chosen_index_list)
        else:

...
def init_ff(dataset='all',phase='test',comp='raw'):
	assert dataset in ['all','Deepfakes','Face2Face','FaceSwap','NeuralTextures','Original']
	original_path=f'data/FaceForensics++/original_sequences/youtube/{comp}/videos/'
	...
	folder_list=[]
	for fake in fakes:
		fake_path=f'data/FaceForensics++/manipulated_sequences/{fake}/{comp}/videos/'
...
def init_dfd(comp='raw'):
	real_path=f'data/FaceForensics++/original_sequences/actors/{comp}/videos/*.mp4'
	real_videos=sorted(glob(real_path))
	fake_path=f'data/FaceForensics++/manipulated_sequences/DeepFakeDetection/{comp}/videos/*.mp4'
